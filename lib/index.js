...
/** @type {{ typescript?: boolean, stylisticTs?: boolean, prettier?: boolean }} */
 ...
 * @returns {Array<any>}
 * */
export default function hagemanto(userOptions) {
	...
	/** @type {any} */
	let config = [presets.builtin, ...presets.base]
	if (options.typescript) {
		config = [...config, ...presets.typescript]
	}
	if (options.stylisticTs) {
		config = [...config, ...presets.stylisticTs]
	}
	if (options.prettier) {
		config = [...config, ...presets.prettier]
	}
	config = [...config, presets.miscellaneous]
