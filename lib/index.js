...
/**@param {import("./index").HagemantoOptions} param0 */
export default function hagemanto({
	...
	if (styler === "prettier")
		import("eslint-plugin-prettier/recommended").then(prettier => {
			config.push({
	...
	config.push({
		...
		rules: {
			// should be deprecated but for some reason it's turned on by defaut
			"indent": "off",
	...
	if (styler === "stylistic")
		import("@stylistic/eslint-plugin").then(stylistic => {
			config.push({
				...stylistic.default.configs.customize({
					indent: "tab",
					quotes: "double",
					semi: true,
					jsx: true,
	...
	if (enableTs)
		import("typescript-eslint").then(tseslint => {
			for (const c of tseslint.default.configs.strictTypeChecked) {
				config.push({ ...c, name: `hagemanto/${c.name?.replace("eslint-recommended", "recommended")}` });
			}

			for (const c of tseslint.default.configs.stylisticTypeChecked) {
	...
	if (enableTailwind)
		import("eslint-plugin-tailwindcss")
			.then(tailwind => {
				for (const c of tailwind.default.configs["flat/recommended"]) {
					config.push({ ...c, name: `hagemanto/${c.name?.replace(":", "/")}` });
				}
			})


	if (enableDefaultLanguageOptions) {
		config.push({
			name: "hagemanto/language-options",
			languageOptions: {
				parserOptions: {
					ecmaVersion: "latest",
					...
					extraFileExtensions: vueConfig ? [".vue"] : [],
					projectService: true,
					// @ts-expect-error
	...
	if (vueConfig) {
		for (const c of vueConfig) {
			config.push({ ...c, name: `hagemanto/${c.name ?? "vue-recommended"}` });
		}

		config.push({
			name: "hagemanto/vue/fine-tuned",
			rules: {
